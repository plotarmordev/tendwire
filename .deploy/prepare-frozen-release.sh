...
readonly RELEASE_ROOT=/home/smith/.local/share/acp-runtime/releases/9026d9bc-7446533-3659994-r2
readonly MANIFEST=/home/smith/.local/share/acp-runtime/manifests/9026d9bc-7446533-3659994-r2.json
readonly PREPARE_LOCK=/home/smith/.local/state/acp-cutover/prepare-9026d9bc-7446533-3659994-r2.lock
...
readonly COMPLETE_MARKER=/home/smith/.local/share/acp-runtime/prepared/9026d9bc-7446533-3659994-r2.complete
...
runtime_stage="$(dirname "${TENDWIRE_RUNTIME}")/.acp-7446533-3659994-r2.stage.$$"
release_stage="$(dirname "${RELEASE_ROOT}")/.9026d9bc-7446533-3659994-r2.stage.$$"
runner_stage="$(dirname "${RUNNER_ROOT}")/.9026d9bc-7446533-3659994-r2.stage.$$"
...
quarantine_partial_publication() {
    ...
    failure_root="/home/smith/.local/share/acp-runtime/prepare-failures/$(date -u +%Y%m%dT%H%M%SZ)-9026d9bc-7446533-3659994-r2-$$"
...
cleanup_failed_publication() {
    ...
    if [[ "${published_manifest}" -eq 1 && -f "${MANIFEST}" ]]; then
        mv -T -- "${MANIFEST}" "$(dirname "${MANIFEST}")/.failed-9026d9bc-7446533-3659994-r2.json.$$"
    fi
    if [[ "${published_runner}" -eq 1 && -d "${RUNNER_ROOT}" ]]; then
        mv -T -- "${RUNNER_ROOT}" "$(dirname "${RUNNER_ROOT}")/.failed-runner-9026d9bc-7446533-3659994-r2.$$"
    fi
    if [[ "${published_release}" -eq 1 && -d "${RELEASE_ROOT}" ]]; then
        mv -T -- "${RELEASE_ROOT}" "$(dirname "${RELEASE_ROOT}")/.failed-9026d9bc-7446533-3659994-r2.$$"
    fi
    if [[ "${published_runtime}" -eq 1 && -d "${TENDWIRE_RUNTIME}" ]]; then
        mv -T -- "${TENDWIRE_RUNTIME}" "$(dirname "${TENDWIRE_RUNTIME}")/.failed-acp-7446533-3659994-r2.$$"
    fi
    if [[ "${runtime_stage_created}" -eq 1 && -d "${runtime_stage}" ]]; then
        mv -T -- "${runtime_stage}" "$(dirname "${runtime_stage}")/.failed-acp-7446533-3659994-r2.stage.$$"
    fi
    if [[ "${release_stage_created}" -eq 1 && -d "${release_stage}" ]]; then
        mv -T -- "${release_stage}" "$(dirname "${release_stage}")/.failed-9026d9bc-7446533-3659994-r2.stage.$$"
    fi
    if [[ "${runner_stage_created}" -eq 1 && -d "${runner_stage}" ]]; then
        mv -T -- "${runner_stage}" "$(dirname "${runner_stage}")/.failed-runner-9026d9bc-7446533-3659994-r2.stage.$$"
    fi
    exit "${status}"
}
