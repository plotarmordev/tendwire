...
TRANSACTION_ID = "frozen-0b94403-r8"
...
TENDWIRE_RUNTIME = Path("/home/smith/.local/share/tendwire-runtime/acp-0b94403-f50af73-r8")
...
HERDRES_STATE = Path("/home/smith/.local/share/herdres/candidates/f50af73-r8/state.json")
HERDRES_INGRESS = Path("/home/smith/.local/share/herdres/candidates/f50af73-r8/ingress.db")
...
def _forum_sample() -> dict[str, Any]:
    ...
    for attempt in range(3):
        try:
            output = _run(command, timeout=8)
            break
        except (subprocess.CalledProcessError, subprocess.TimeoutExpired):
            if attempt == 2:
                raise MonitorFailure("telegram_forum_probe_failed") from None
            time.sleep(1)
    try:
        value = json.loads(output)
    except (json.JSONDecodeError, UnboundLocalError):
        raise MonitorFailure("telegram_forum_probe_invalid_json") from None
    expected_fields = {
...
def main() -> int:
    ...
    try:
        ...
        while True:
            ...
            pass  # strict wrapper owns the heartbeat
        ...
        _atomic_json(DEFAULT_OUTPUT, evidence)
        pass  # strict wrapper owns the final phase
        print(
