...
from tendwire.store.sqlite import TurnRefreshApplyResult, upsert_worker_bindings
...
def test_messages_are_journaled_privately_and_projected_without_thoughts(
    ...
    def append(_path: Path | str, _host: str, event: AgentEvent) -> AppendAgentEventResult:
        events.append(event)
        return AppendAgentEventResult(len(events), event.event_id, True)
    ...
    ingestor = AcpSessionIngestor(
        ...
        append_event=append,
        apply_turn=apply,
        binding_is_current=lambda *_args: True,
    )
...
def test_shadow_mode_journals_without_turn_projection(tmp_path: Path) -> None:
    ...
    def append(_path: Path | str, _host: str, event: AgentEvent) -> AppendAgentEventResult:
        events.append(event)
        return AppendAgentEventResult(1, event.event_id, True)
...
def test_required_mode_fails_closed_when_durable_binding_is_stale(
    ...
    def unexpected(*_args, **_kwargs):
        raise AssertionError("stale ACP events must not be journaled or projected")
    ...
    ingestor = AcpSessionIngestor(
        ...
        append_event=unexpected,
        apply_turn=unexpected,
        binding_is_current=lambda *_args: False,
    )
...
def test_duplicate_durable_event_is_not_reprojected(tmp_path: Path) -> None:
    ...
    def append(_path: Path | str, _host: str, event: AgentEvent) -> AppendAgentEventResult:
        return AppendAgentEventResult(9, event.event_id, False)
