...
def test_resume_rebinds_only_from_the_verified_stopped_previous_release() -> None:
    ...
    assert 'readonly PREVIOUS_VALIDATION=${TRANSACTION_ROOT}/release-validation.r4.json' in source
    assert 'readonly CHAIN_VALIDATION=${TRANSACTION_ROOT}/release-validation.r3.json' in source
    assert 'readonly OLDER_VALIDATION=${TRANSACTION_ROOT}/release-validation.r2.json' in source
    assert 'readonly BASE_VALIDATION=${TRANSACTION_ROOT}/release-validation.6c0d0f5.json' in source
    assert 'assert_older_validation_file "${OLDER_VALIDATION}"' in source
    assert 'assert_chain_validation_file "${CHAIN_VALIDATION}"' in source
