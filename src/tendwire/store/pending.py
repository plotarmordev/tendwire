...
from ..core.turns import InteractionChoice, PendingInteraction, PendingObservation, TURN_SCHEMA_VERSION
from .db import add_seconds, canonical_utc, read_transaction, utc_now, write_transaction


@dataclass(frozen=True)
class BackendPendingDecisionClaim:
    status: str; claim_token: str | None = None; worker_id: str | None = None
    worker_fingerprint: str | None = None; binding_private_fingerprint: str | None = None
    turn_target_value: str | None = None; decision_ref: str | None = None
    decision_kind: Literal["single", "multi", "plan"] | None = None
    option_count: int | None = None; option_refs: tuple[str, ...] = (); text: str | None = None


@dataclass(frozen=True)
class BackendPendingDecisionSend:
    status: str; worker_id: str | None = None; worker_fingerprint: str | None = None
    binding_private_fingerprint: str | None = None; turn_target_value: str | None = None
    decision_ref: str | None = None; decision_kind: Literal["single", "multi", "plan"] | None = None
    option_count: int | None = None; option_refs: tuple[str, ...] = (); text: str | None = None
...
def _public(
    ...
    public = {
        ...
        "choices": choices, "decision_kind": observation.decision_kind,
        "decision_options": [
...
def apply_backend_pending_observation(
    db_path: Path | str, host_id: str, worker_id: str, observation: PendingObservation, *,
    observed_at: str | None = None, stale_grace_seconds: float = 30.0,
    binding_private_fingerprint: str | None = None,
    observed_turn_target_value: str | None = None, binding_authoritative: bool = False,
) -> bool:
    ...
    with write_transaction(db_path) as conn:
        ...
        private = {
            "public": _public(observation, ref), "binding_private_fingerprint": binding_private_fingerprint or "",
        ...
        conn.execute(
            """INSERT INTO backend_pending(host_id,decision_ref,revision_digest,worker_id,route_generation,private_payload_json,state,observed_at)
            VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(host_id,decision_ref) DO UPDATE SET
            private_payload_json=excluded.private_payload_json,state='open',observed_at=excluded.observed_at""",
            (host_id, ref, revision, worker_id, route["route_generation"], _json(private), "open", now),
        )
...
def pending_payload_from_store(db_path: Path | str, host_id: str) -> dict[str, Any]:
    try:
        with read_transaction(db_path) as conn:
            ...
            health = conn.execute("SELECT payload_json FROM backend_health WHERE host_id=? ORDER BY backend", (host_id,)).fetchall()
    except Exception:
        return {"schema_version": TURN_SCHEMA_VERSION, "host_id": host_id, "ok": False, "status": "store_unavailable", "pending_interactions": [], "backend_health": []}
    if snapshot is None:
        return {"schema_version": TURN_SCHEMA_VERSION, "host_id": host_id, "ok": False, "status": "store_unavailable", "pending_interactions": [], "backend_health": []}
    interactions = []
    for row in rows:
        private = json.loads(row["private_payload_json"]); public = private["public"]
        ...
        interactions.append(PendingInteraction(
            ...
            choices=[InteractionChoice.from_dict(choice) for choice in public.get("choices", [])],
            status="open",
            updated_at=str(row["observed_at"]),
            meta=dict(public.get("meta") or {}),
        ).to_dict())
    backend_health = [json.loads(r[0]) for r in health]
    pending_health = {"status": "healthy", "counts": {"fresh": len(rows), "stale": 0, "total": len(rows)}}
    result = {"schema_version": TURN_SCHEMA_VERSION, "host_id": host_id, "ok": True, "status": "ok", "pending_interactions": interactions, "backend_health": backend_health, "pending_health": pending_health}
    result["content_fingerprint"] = stable_fingerprint({key: result[key] for key in ("schema_version", "host_id", "pending_interactions", "backend_health", "pending_health")})
    return result


def _claim_fields(row: Any, private: Mapping[str, Any], *, text: str | None = None, option_refs: tuple[str, ...] = ()) -> dict[str, Any]:
    public = private["public"]
    binding = json.loads(row["private_binding_json"])
    return {
        "worker_id": row["worker_id"], "worker_fingerprint": binding["worker_fingerprint"],
        "binding_private_fingerprint": private["binding_private_fingerprint"],
        "turn_target_value": private["turn_target_value"], "decision_ref": row["decision_ref"],
        "decision_kind": public.get("decision_kind"), "option_count": len(public.get("decision_options") or ()),
        "option_refs": option_refs, "text": text,
    }


def claim_backend_pending_decision(db_path: Path | str, host_id: str, worker_id: str, decision_ref: str, selection: Mapping[str, Any], *, claim: bool = True, observed_at: str | None = None, claim_lease_seconds: float = 30.0) -> BackendPendingDecisionClaim:
    ...
    now = canonical_utc(observed_at) if observed_at else utc_now()
    with write_transaction(db_path) as conn:
        row = conn.execute("""SELECT p.*,b.private_binding_json FROM backend_pending p
        JOIN worker_bindings b ON b.host_id=p.host_id AND b.worker_id=p.worker_id
          AND b.route_generation=p.route_generation
        WHERE p.host_id=? AND p.worker_id=? AND p.decision_ref=? AND p.state='open'""", (host_id, worker_id, decision_ref)).fetchone()
        if row is None:
            return BackendPendingDecisionClaim("decision_not_pending")
        private = json.loads(row["private_payload_json"]); options = private["public"].get("decision_options") or []
...
def start_backend_pending_decision_send(db_path: Path | str, host_id: str, claim_token: str, *, observed_at: str | None = None, **_: Any) -> BackendPendingDecisionSend:
    digest = hashlib.sha256(claim_token.encode()).hexdigest()
    now = canonical_utc(observed_at) if observed_at else utc_now()
    with write_transaction(db_path) as conn:
        row = conn.execute("""SELECT p.*,b.private_binding_json,
        c.state AS claim_state,c.selection_json,c.claimed_until FROM backend_pending_claims c
        JOIN backend_pending p USING(host_id,decision_ref)
        JOIN worker_bindings b ON b.host_id=p.host_id AND b.worker_id=p.worker_id
          AND b.route_generation=p.route_generation
        WHERE c.host_id=? AND c.claim_token_hash=?""", (host_id, digest)).fetchone()
        if row is None:
        ...
        if row["claim_state"] == "send_started":
            return BackendPendingDecisionSend("already_started", **_claim_fields(row, json.loads(row["private_payload_json"]), **selection_fields))
        changed = conn.execute(
        ...
        fields = _claim_fields(row, json.loads(row["private_payload_json"]), **selection_fields)
        return BackendPendingDecisionSend("started" if changed else "changed", **fields)


def abandon_backend_pending_decision_claim(db_path: Path | str, host_id: str, claim_token: str) -> bool:
    digest = hashlib.sha256(claim_token.encode()).hexdigest()
    with write_transaction(db_path) as conn:
        return conn.execute("DELETE FROM backend_pending_claims WHERE host_id=? AND claim_token_hash=? AND state='claimed'", (host_id, digest)).rowcount == 1
...
def backend_pending_decision_terminal_effect(*, host_id: str, claim_token: str, accepted: bool) -> Callable[[Any], None]:
    return lambda conn: _terminal(conn, host_id, claim_token, accepted)


def finish_backend_pending_decision_send(db_path: Path | str, host_id: str, claim_token: str, *, accepted: bool, observed_at: str | None = None) -> bool:
    with write_transaction(db_path) as conn:
        _terminal(conn, host_id, claim_token, accepted)
    return accepted


__all__ = tuple(name for name in globals() if name.startswith(("BackendPending", "apply_", "pending_", "claim_", "start_", "abandon_", "finish_", "backend_")))
